...
namespace FOS\Bundle\OAuthSocialConnectBundle\DependencyInjection\Security\Factory;
...
class OAuthFactory extends AbstractFactory
{
    ...
    protected function createResourceOwnerMap(ContainerBuilder $container, $id, array $config)
    {
        ...
        $container->setParameter('fos_oauth_social_connect.resource_ownermap.configured.'.$id, $resourceOwnersMap);

        $container
            ->setDefinition($this->getResourceOwnerMapReference($id), new $definitionClassname('fos_oauth_social_connect.abstract_resource_ownermap'))
            ->replaceArgument(2, new Parameter('fos_oauth_social_connect.resource_ownermap.configured.'.$id))
            ->setPublic(true)
        ;
    }
    ...
    {
        return new Reference('fos_oauth_social_connect.resource_ownermap.'.$id);
    }
    ...
    {
        $definitionClassname = $this->getDefinitionClassname();
        $providerId = 'fos_oauth_social_connect.authentication.provider.oauth.'.$id;

        $this->createResourceOwnerMap($container, $id, $config);

        $container
            ->setDefinition($providerId, new $definitionClassname('fos_oauth_social_connect.authentication.provider.oauth'))
            ...
            ->addArgument(new Reference('fos_oauth_social_connect.user_checker'))
    ...
    {
        $definitionClassname = $this->getDefinitionClassname();
        $serviceId = 'fos_oauth_social_connect.user.provider.entity.'.$id;

        // todo: move this to factories?
        switch (key($config)) {
            case 'oauth':
                $container
                    ->setDefinition($serviceId, new $definitionClassname('fos_oauth_social_connect.user.provider'))
                ;
                break;
            case 'orm':
                $container
                    ->setDefinition($serviceId, new $definitionClassname('fos_oauth_social_connect.user.provider.entity'))
    ...
    {
        $entryPointId = 'fos_oauth_social_connect.authentication.entry_point.oauth.'.$id;
        $definitionClassname = $this->getDefinitionClassname();

        $container
            ->setDefinition($entryPointId, new $definitionClassname('fos_oauth_social_connect.authentication.entry_point.oauth'))
    ...
    {
        return 'fos_oauth_social_connect.authentication.listener.oauth';
    }
